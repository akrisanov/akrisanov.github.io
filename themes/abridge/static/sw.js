class Pwa {
    ...
    constructor(self) {
        ...
        // Minimal, site-specific pre-cache. The SW will cache additional assets on-demand.
        this.BASE_CACHE_FILES = [
            ...
            '/favicon-16x16.png',
            '/favicon.svg',
            // Search indexes for en/ru
            '/search_index.en.json',
            '/search_index.ru.json',
        ];
                ...
                caches.open(this.CACHE_NAME).then(async cache => {
                    ...
                    return fetch(event.request.clone()).then(resp => {
                        ...
                        try {
                            ...
                            const offlinePath = reqUrl.pathname.startsWith('/ru/') ? '/ru/offline/' : this.OFFLINE_PAGE;
                            return cache.match(offlinePath).then(m => m || cache.match(this.OFFLINE_PAGE));
                        } catch (_) {

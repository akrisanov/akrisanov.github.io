...
description = "A production guide to monitoring vLLM 0.23.x with Prometheus and Grafana, including key metrics, PromQL, alerts, and runbooks."
date = 2026-01-28
updated = 2026-06-18
draft = false
...
A vLLM server can pass Kubernetes health checks while users experience slow or unstable responses. Requests may wait
in the scheduler, long prompts may make prefill expensive, or token streaming may become uneven.

This guide focuses on the metrics needed to diagnose latency, scheduler backlog, KV-cache pressure, throughput changes,
and workload shifts during incidents and capacity reviews. The examples use Prometheus and Grafana.
...
A user-visible latency regression can originate in any of these layers. Monitor all three groups:
...
Engine metrics explain what happens inside vLLM, but do not show whether the complete service is healthy.
...
The examples below group by `model_name`. Add stable labels such as `cluster`, `namespace`, or `service` as needed.
Avoid aggregating unrelated models into one percentile: a fast small model can hide a regression in a much larger one.

## User-visible latency metrics

Three latency metrics cover most user-visible problems.
...
A high TTFT may indicate queueing, longer prompts, lower prefix-cache reuse, or a slow first model-execution step. It does
not by itself show that the GPU is slow.
...
Use them to detect longer prompts or outputs, measure how prefix caching changes prefill work, and distinguish traffic
composition changes from infrastructure regressions.

Keep workload-shape panels near latency and throughput panels so that workload changes are not mistaken for serving regressions.
...
A non-zero waiting gauge is not necessarily an incident. Short queues are normal under bursty traffic and
...
High usage is not necessarily a problem. A healthy operating range depends on model architecture, context-length distribution,
...
An occasional preemption during a short burst may not justify an incident. Sustained preemption above a tested baseline
matters, especially when latency or queueing also degrades.
...
An incident dashboard should show user-visible latency, queued work, memory pressure, and workload changes.
...
For availability and latency SLOs, prefer multi-window burn-rate alerts. Raw percentile alerts are included here as
compact examples, but should not replace an SLO-based alerting strategy.
...
- [vLLM metrics design](https://docs.vllm.ai/en/v0.23.0/design/metrics/)
  <span class="further-reading-description">Design notes for the vLLM metrics system and its collectors.</span>
